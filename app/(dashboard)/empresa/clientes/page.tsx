...
import { Plus, Loader2, Pencil, Trash2, Users, Mail, Phone, MapPin, FileText } from 'lucide-react'
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-app">Clientes</h1>
          <p className="mt-1 text-sm text-app-base">
            Cadastro de clientes — {business?.name ?? ''}
          </p>
        </div>
        <Button
      ...
      {loadingList ? (
        ...
        <EmptyState
          ...
          description="Adicione clientes para associa-los ao seu faturamento e despesas."
          action={{ label: 'Adicionar cliente', onClick: () => setModalOpen(true) }}
        />
      ) : (
            ...
            <div key={c.id} className="panel-card p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-app">{c.name}</p>
                  {c.legal_name && (
                    <p className="mt-0.5 text-xs text-app-soft">{c.legal_name}</p>
                  )}
                  ...
                  {c.notes && (
                    <p className="mt-2 text-xs text-app-soft">{c.notes}</p>
                  )}
                </div>
