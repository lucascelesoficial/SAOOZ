...
function formatPlanStatusLabel(
  ...
  if (lifecycleStatus === 'active') return 'Ativo'
  if (lifecycleStatus === 'trialing') return `${trialDaysRemaining} dia(s)`
  if (lifecycleStatus === 'past_due') return 'Pagamento pendente'
  if (lifecycleStatus === 'canceled') return 'Cancelado'
  if (lifecycleStatus === 'expired') return 'Expirado'
  return 'Inativo'
}
