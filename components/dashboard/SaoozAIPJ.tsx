...
export function SaoozAIPJ({ userId }: SaoozAIPJProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'ai',
      text: `${business?.name ?? 'Empresa'} carregada. Posso analisar e preparar lançamentos para sua confirmação.`,
    },
  ])
  ...
  async function send(text: string) {
    ...
    try {
      ...
      if (!response.ok || data.error) {
        throw new Error(data.error || 'Erro ao processar a IA empresarial.')
      }

      const replyText =
        typeof data.text === 'string' && data.text.trim()
          ? data.text.trim()
          : 'Pronto. Analisei o contexto empresarial.'
    ...
    >
      <div
        ...
        style={{
          ...
          background: 'color-mix(in oklab, var(--accent-cyan) 3%, transparent)',
        }}
      >
        <button
          onClick={toggleVoice}
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold leading-none text-app">
            SAOOZ <span style={{ color: 'var(--accent-cyan)' }}>AI</span>
          </p>
          <p
